...
class Music:
    ...
    async def play(self, ctx, *, song : str): # !!play
        ...
        try:
            ...
            await bot.edit_message(tmp, ":pistol: A fatal error occurred. Please report this at https://discord.gg/eDRnXd6")
        except Exception as e:
            e = str(e)
            print(color.RED + "ERROR: " + e)
            errors += 1
...
class General:
    ...
    async def ping(self, ctx): # !!ping
        ...
        await self.bot.send_typing(ctx.message.channel)
        start = time.time()
        res = os.system("ping gateway.discord.gg -n 1")
        _time = round((time.time() - start) * 1000)
        embed = discord.Embed()
...
@bot.event
async def on_voice_state_update(before, after): # Voice state update
...
async def on_message(message): # When a message is sent
    ...
    elif message.content.lower() == "!!user": # !!user
